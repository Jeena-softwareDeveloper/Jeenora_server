...
class GuideController {
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            try {
                ...
                const guide = await Guide.create({
                    ...
                    slug,
                    level,
                    difficulty: level, // Sync level to difficulty for frontend
                    secondHeading,
                    description,
                    content: fields.content,
                    scientificAnalysis: fields.scientificAnalysis,
                    steps: fields.steps ? JSON.parse(fields.steps) : [],
                    maintenance: fields.maintenance ? JSON.parse(fields.maintenance) : [],
                    troubleshooting: fields.troubleshooting ? JSON.parse(fields.troubleshooting) : [],
                    benefits: fields.benefits ? JSON.parse(fields.benefits) : [],
                    readTime: fields.readTime,
                    image: imageUrl,
                    districts: fields.districts ? JSON.parse(fields.districts) : ['All'],
                    crops: fields.crops ? JSON.parse(fields.crops) : ['All'],
                    isActive: true
                });
    ...
    get_guides = async (req, res) => {
        try {
            ...
            if (district) {
                query.districts = { $in: [district, 'All'] };
            }

            if (crop) {
                query.crops = { $in: [crop, 'All'] };
            }
            ...
            let guides = await Guide.find(query).populate('category').sort({ createdAt: -1 });
    ...
    get_guide_by_slug = async (req, res) => {
        try {
            const { slug } = req.params;
            const guide = await Guide.findOne({ slug, isActive: true }).populate('category');
            ...
            return responseReturn(res, 200, { guide: guideObj });
        } catch (error) {
            return responseReturn(res, 500, { error: error.message });
        }
    }
    ...
    get_guides_by_category = async (req, res) => {
        try {
            ...
            let guides = await Guide.find({ category: categoryId, isActive: true }).populate('category');
            guides = guides.map(g => {
                const guideObj = g.toObject();
                if (guideObj.image && guideObj.image.includes("http://")) {
                    guideObj.image = guideObj.image.replace("http://", "https://");
    ...
    update_guide = async (req, res) => {
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            try {
                ...
                if (fields.content) guide.content = fields.content;
                if (fields.scientificAnalysis) guide.scientificAnalysis = fields.scientificAnalysis;
                if (fields.steps) guide.steps = JSON.parse(fields.steps);
                if (fields.maintenance) guide.maintenance = JSON.parse(fields.maintenance);
                if (fields.troubleshooting) guide.troubleshooting = JSON.parse(fields.troubleshooting);
                if (fields.benefits) guide.benefits = JSON.parse(fields.benefits);
                if (fields.readTime) guide.readTime = fields.readTime;
                if (fields.districts) guide.districts = JSON.parse(fields.districts);
                if (fields.crops) guide.crops = JSON.parse(fields.crops);
