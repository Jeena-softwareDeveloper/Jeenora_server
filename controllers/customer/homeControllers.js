const category = require('../../models/customer/Category')
const { responseReturn } = require("../../utils/response")
const productModel = require('../../models/partner/Product')
const queryProducts = require('../../utils/queryProducts')
const moment = require('moment')
const reviewModel = require('../../models/customer/reviewModel')
...
class homeControllers {
    ...
    get_categorys = async (req, res) => {
        try {
            const wearCategoryModel = require('../../models/customer/WearCategory');
    ...
    product_details = async (req, res) => {
        ...
        try {
            const partnerSelection = 'name shopInfo businessDetails image status';
            let product = await productModel.findOne({ slug }).populate({ path: 'offers', match: { status: 'active' } }).populate('partnerId', partnerSelection);

            if (!product && ObjectId.isValid(slug)) {
                product = await productModel.findById(slug).populate({ path: 'offers', match: { status: 'active' } }).populate('partnerId', partnerSelection);
            }
    ...
    validate_recent_products = async (req, res) => {
        ...
        try {
            ...
            const products = await productModel.find({ _id: { $in: productIds }, status: 'active' })
                .select('name price listingPrice originalPrice mrp images slug category subCategory _id variants shopName partner rating discount');
    ...
    get_recent_products = async (req, res) => {
        ...
        try {
            ...
            let user = await wearBuyerModel.findById(userId).populate({
                path: 'recentViewed',
                select: 'name productName price listingPrice originalPrice mrp images slug category subCategory _id variants shopName partner rating discount'
            });
