...
class wearCatalogController {
    ...
    add_catalog = async (req, res) => {
        ...
        try {
            ...
            for (const item of productsToCreate) {
                ...
                const WearCategory = require('../../models/customer/WearCategory');
                let catDoc = null;
                let subCatDoc = null;
    ...
    get_my_catalogs = async (req, res) => {
        ...
        try {
            ...
            const partnerObjectId = new mongoose.Types.ObjectId(String(supplier._id));
            ...
            const legacyProductsRaw = await productModel.find({ partnerId: partnerObjectId }).sort({ createdAt: -1 }).lean();
    ...
    get_all_catalogs = async (req, res) => {
        try {
            ...
            const wearCatalogs = groupedCatalogs.map(g => {
                const partnerInfo = (g.supplierInfo && g.supplierInfo.length > 0)
                    ? g.supplierInfo[0]
                    : (g.partnerInfo && g.partnerInfo.length > 0 ? g.partnerInfo[0] : { _id: g.partnerId, businessDetails: { shopName: 'Unknown Partner' } });
            ...
            const legacyCatalogs = legacyProductsRaw.map(p => ({
                ...p,
                productName: p.name,
                partnerId: supplierMap.get(p.partnerId?.toString()) || { _id: p.partnerId, businessDetails: { shopName: p.shopName || 'Legacy Partner' } },
                catalogId: p._id,
                variants: p.variants || [{
    ...
    get_manual_list = async (req, res) => {
        ...
        try {
            if (!partnerId) return responseReturn(res, 400, { error: 'Partner ID is required' });
    ...
    get_supplier_catalogs = async (req, res) => {
        const { partnerId } = req.query;
        try {
            if (!partnerId) return responseReturn(res, 400, { error: 'partnerId is required' });
            const productModel = require('../../models/partner/Product');
            ...
            // 2. Get Legacy Products for this seller
            const legacyProductsRaw = await productModel.find({ partnerId: new mongoose.Types.ObjectId(partnerId) }).sort({ createdAt: -1 }).lean();
    ...
    scan_catalog_product = async (req, res) => {
        ...
        try {
            ...
            if (!product && query.length === 8) {
                 const allProducts = await WearProduct.find({ partnerId: supplier._id }, '_id');
    ...
    get_catalog_by_id = async (req, res) => {
        ...
        try {
            ...
            let query = { partnerId: supplier._id };
            ...
            let products = await WearProduct.find(query).lean();
    ...
    supplier_edit_catalog = async (req, res) => {
        ...
        try {
            ...
            // Verify ownership
            let ownProducts = await WearProduct.find({ catalogId, partnerId: supplier._id });
            
            // Fallback for missing/legacy catalogId
            if ((!ownProducts || ownProducts.length === 0) && catalogId.length === 24) {
                const singleProduct = await WearProduct.findOne({ _id: catalogId, partnerId: supplier._id });
