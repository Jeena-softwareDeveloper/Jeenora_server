...
const reviewModel = require('../../models/customer/wearReviewModel');
...
class supplierController {
    ...
    get_supplier_dashboard_data = async (req, res) => {
        ...
        try {
            ...
            const totalCatalogs = await WearProduct.countDocuments({ partnerId: supplier._id });
            const activeCatalogs = await WearProduct.countDocuments({ partnerId: supplier._id, status: 'active' });
            const pendingCatalogs = await WearProduct.countDocuments({ partnerId: supplier._id, status: 'pending' });
    ...
    get_supplier_payouts = async (req, res) => {
        ...
        try {
            ...
            const orders = await authOrderModel.find({ partnerId: supplier._id, delivery_status: 'delivered' }).sort({ updatedAt: -1 });

            // In a real system, you'd have a separate payout table. 
            // For now, we derive it from delivered orders.
            const commissionRate = 0.00; // 0% platform fee (Jeenora doesn't take commission per product)
    ...
    get_suppliers = async (req, res) => {
        ...
        try {
            ...
            // 2. Get Partners (Legacy)
            let partnerQuery = {};
            if (searchValue) {
                partnerQuery['name'] = { $regex: searchValue, $options: 'i' };
            }
            if (status && status !== 'all') {
                partnerQuery.status = status;
            }

            const partnersRaw = await Partner.find(partnerQuery).lean();

            // 3. Map Partners to Supplier format for UI consistency
            const legacySuppliers = partnersRaw.map(s => ({
                _id: s._id,
            ...
            const legacyProductModel = require('../../models/partner/Product');
            ...
            const [wearCounts, legacyCounts] = await Promise.all([
                WearProduct.aggregate([
                    { $match: { partnerId: { $in: allVendorObjectIds } } },
            ...
            const [newWearProducts, newLegacyProducts] = await Promise.all([
                WearProduct.find({ 
                    partnerId: { $in: allVendorObjectIds },
            ...
            newWearProducts.forEach(p => newProductMap.set(p.partnerId.toString(), true));
            newLegacyProducts.forEach(p => newProductMap.set(p.partnerId.toString(), true));
    ...
    update_supplier = async (req, res) => {
        ...
        try {
            ...
            if ((ownerName || req.body.phone) && supplier.user) {
                const User = require('../../models/customer/wearBuyerModel');
                const updateData = {};
    ...
    get_supplier_by_id = async (req, res) => {
        ...
        try {
            ...
            const partnerObjId = new mongoose.Types.ObjectId(supplierId);

            // Catalog counts
            const [totalCatalogs, activeCatalogs, pendingCatalogs, legacyCount] = await Promise.all([
                WearProduct.countDocuments({ partnerId: partnerObjId }),
                WearProduct.countDocuments({ partnerId: partnerObjId, status: 'active' }),
                WearProduct.countDocuments({ partnerId: partnerObjId, status: 'pending' }),
                legacyProductModel.countDocuments({ partnerId: partnerObjId }),
            ]);

            // Sales stats from authOrders
            const orders = await authOrderModel.find({ partnerId: partnerObjId }).lean();
    ...
    send_verification_email = async (req, res) => {
        ...
        try {
            ...
            const WearEmailOtp = require('../../models/admin/wearEmailOtpModel');
    ...
    get_financial_dashboard = async (req, res) => {
        try {
            ...
            const orders = await authOrderModel.find({ partnerId: supplier._id });
    ...
    get_price_recommendations = async (req, res) => {
        try {
            ...
            const products = await WearProduct.find({ partnerId: supplier._id, status: 'active' }).limit(5);
    ...
    get_pricing_data = async (req, res) => {
        const { id } = req;
        try {
            const products = await WearProduct.find({ partnerId: id });
    ...
    get_price_recommendations = async (req, res) => {
        const { id } = req;
        try {
            const products = await WearProduct.find({ partnerId: id }).limit(10);
