const WearOfferCampaign = require("../../models/partner/wearOfferCampaignModel");
const WearNotification = require("../../models/admin/WearNotification");
const Supplier = require("../../models/partner/Supplier");
const WearProductOffer = require("../../models/partner/productOfferModel");
...
class wearOfferController {
    ...
    get_campaign_participants_admin = async (req, res) => {
        ...
        try {
            const participantIds = await WearProduct.distinct('partnerId', { campaignId });
            ...
            const participants = await Promise.all(suppliers.map(async (s) => {
                const count = await WearProduct.countDocuments({ partnerId: s._id, campaignId });
                return {
                    ...s.toObject(),
                    productCount: count
                };
    ...
    add_products_to_campaign = async (req, res) => {
        ...
        try {
            ...
            await WearProduct.updateMany(
                { _id: { $in: productIds }, partnerId: supplier._id },
                { campaignId: campaignId }
            );
    ...
    remove_product_from_campaign = async (req, res) => {
        ...
        try {
            ...
            const product = await WearProduct.findOne({ _id: productId, partnerId: supplier._id });
