...
class B2BOrderController {
    ...
    accept_order = async (req, res) => {
        try {
            ...
            if (order.acceptDeadline && new Date() > order.acceptDeadline) {
                ...
                order.cancel_reason = 'Auto-cancelled: Seller did not accept within 48 hours';
                await order.save();
    ...
    // ── SELLER: Update B2B status (packed → shipped → delivered) ──────────
    ...
    // ── SELLER: Get B2B orders ────────────────────────────────────────────
    get_seller_b2b_orders = async (req, res) => {
        try {
            const sellerId = req.id;
            const { status } = req.query;

            const filter = { sellerId, order_type: 'B2B' };
    ...
    // ── SELLER: Get B2B order detail ──────────────────────────────────────
    get_seller_b2b_order = async (req, res) => {
        try {
            const sellerId = req.id;
            const { orderId } = req.params;

            const order = await authOrderModel.findOne({ _id: orderId, sellerId, order_type: 'B2B' }).lean();
            if (!order) {
