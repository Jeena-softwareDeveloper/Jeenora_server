...
const wearBuyerModel = require("../../models/customer/wearBuyerModel");      // actual Wear app user model
const wearSessionModel = require("../../models/customer/wearSessionModel");    // JWT session tracking
...
class adminWearController {
    ...
    get_financial_stats = async (req, res) => {
        try {
            ...
            const payableStats = await authOrder.aggregate([
                ...
                { $group: { _id: null, totalPayable: { $sum: '$partnerAmount' } } }
            ]);
            ...
            const revByVendor = await authOrder.aggregate([
                { $match: { payment_status: 'paid' } },
                { $group: { _id: '$partnerId', revenue: { $sum: '$price' }, commission: { $sum: '$commissionAmount' } } },
    ...
    get_vendor_sales_report = async (req, res) => {
        ...
        try {
            const stats = await authOrder.aggregate([
                { $match: { partnerId: new ObjectId(supplierId), payment_status: 'paid' } },
                {
                    $group: {
                        _id: null,
                        totalRevenue: { $sum: '$price' },
                        totalCommission: { $sum: '$commissionAmount' },
                        netEarnings: { $sum: '$partnerAmount' },
                        orderCount: { $sum: 1 }
                    }
                }
            ]);
    ...
    remove_duplicate_products = async (req, res) => {
        try {
            // Logic to find products with same name and partner
            const duplicates = await wearProductModel.aggregate([
                { $group: { _id: { name: "$productName", partner: "$partnerId" }, count: { $sum: 1 }, ids: { $push: "$_id" } } },
                { $match: { count: { $gt: 1 } } }
            ]);
    ...
    get_advanced_analytics = async (req, res) => {
        try {
            ...
            const topPerformers = await customerOrder.aggregate([
                { $unwind: '$products' },
                {
                    $group: {
                        _id: '$products._id',
